...
- Release builds no longer pass empty Apple signing variables to Tauri.
...
- Release workflow now uses the active updater signing key and builds the macOS arm64 artifact only.

## [0.2.4] - 2026-04-25

### Added

- In-app update checks and user-approved installs backed by signed GitHub Release artifacts.

### Fixed

- CI dependency audit now uses a patched PostCSS version.
- Release builds no longer require Apple signing/notarization secrets when only unsigned updater artifacts are needed.
