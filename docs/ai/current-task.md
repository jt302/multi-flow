...
- [x] 设置页增加机型预设管理（数据库单源：预设列表/新增/修改），并让环境创建/展示/启动统一走同一预设来源
...
- [x] 环境代理绑定 / 解绑时，后端统一覆盖刷新 Profile 持久化语言 / 时区；运行中环境下次启动生效
...
- [x] 插件页“下载代理”选择已改为应用级本地偏好，刷新页面和重启应用后会恢复上次选择；若代理已失效则自动回退为“不使用代理”
- [x] 插件库卡片新增“在商店中打开”，可直接调用系统默认浏览器打开插件对应的 Chrome Web Store 页面
