...
## 本轮新增（RPA 导航重构 + Task 模型）

- [x] 主导航 `RPA` 改为二级子菜单：`/rpa/flows` / `/rpa/tasks` / `/rpa/runs`，`/rpa` 重定向到流程管理
- [x] 侧边栏接入子菜单渲染（展开态显示子项，折叠态保留一级图标）
- [x] 顶栏命令搜索补充 RPA 子路由快捷跳转
- [x] 新增 RPA 任务数据模型与迁移：`rpa_tasks` / `rpa_task_targets`
- [x] `rpa_runs` 新增 `task_id / task_name` 快照字段，保留历史执行关联
- [x] 新增后端任务服务与命令：任务 CRUD、启停、立即执行、按条件列举运行记录
- [x] 新增调度器：支持 Cron 计划任务（应用运行时触发、不补跑错过窗口）
- [x] 前端新增任务管理页（表单 + 列表 + 启停/执行/删除）
- [x] 前端新增运行记录页（筛选 + 实例/步骤/产物详情）
- [x] 编辑器移除“运行中心”主入口，仅保留“临时调试运行”；正式运行统一走任务管理
