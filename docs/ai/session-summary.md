...
## 2026-04-18 dev/release 数据目录与 Skills 修复

### 已完成

- 后端新增统一数据目录解析；`dev` 固定落到 `app_local_data_dir/dev`，`release` 继续使用原目录
- 数据目录切分已覆盖数据库、日志、资源、偏好、插件包、截图/自动化产物与 `fs` 工作区
- AI Skill 内置目录改为“打包资源优先，仓库目录回退”，并在 Tauri bundle 中加入 `default-skills` 资源
- 环境表单背景色校验改为仅在 `custom` 模式生效；切换到 `inherit / none` 会清除旧错误状态

### 验收重点

- `release` 不迁移旧数据，兼容现有目录
- `dev` 首次启动将使用空的新目录
- Skills 页面应能看到内置 skills
- 创建/编辑环境时选择“跟随分组 / 不设置背景色”可正常提交
