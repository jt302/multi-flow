...
**Goal:** 让 `/rpa` 只展示 active RPA 流程，并把 archived 流程接入现有回收站页，支持恢复与彻底删除。
...
**Files:**
- Create: `src/features/rpa/model/rpa-flow-list.test.ts`
- Create: `src/features/recycle-bin/model/rpa-recycle-bin.test.ts`

**Step 1: Write the failing test**

- 验证 `/rpa` 列表 helper 只返回 `active` 流程，且按 `updatedAt` 倒序。
- 验证回收站 helper 会把 RPA deleted 项计入总数，并只返回 deleted 流程。

**Step 2: Run test to verify it fails**

Run: `pnpm exec tsx --test src/features/rpa/model/rpa-flow-list.test.ts src/features/recycle-bin/model/rpa-recycle-bin.test.ts`
...
**Files:**
- Create: `src/features/rpa/model/rpa-flow-list.ts`
- Create: `src/features/recycle-bin/model/rpa-recycle-bin.ts`

**Step 1: Write minimal implementation**

- 提供 `/rpa` 列表过滤排序 helper
...
- Modify: `src/features/rpa/model/use-rpa-actions.ts`
- Modify: `src/features/rpa/ui/rpa-page.tsx`
...
Run:
- `pnpm exec tsx --test src/features/rpa/model/rpa-flow-list.test.ts src/features/recycle-bin/model/rpa-recycle-bin.test.ts src/features/rpa/model/rpa-editor-window.test.ts src/features/rpa/model/rpa-editor-close-guard.test.ts`
