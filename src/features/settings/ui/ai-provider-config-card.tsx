...
import type { AiConfigEntry, AiProviderType } from '@/entities/automation/model/types';
...
export function AiProviderConfigCard() {
	...
	const createMutation = useMutation({
		...
		onError: (e) => toast.error(t('settings:ai.addFailed', { error: String(e) })),
	});
	...
	const updateMutation = useMutation({
		...
		onError: (e) => toast.error(t('settings:ai.updateFailed', { error: String(e) })),
	});
	...
	const deleteMutation = useMutation({
		...
		onError: (e) => toast.error(t('settings:ai.deleteFailed', { error: String(e) })),
	});
								...
								>
									<div className="min-w-0 flex-1">
										...
										<div className="text-xs text-muted-foreground truncate">
											{aiProviders.find((p) => p.value === entry.provider)?.label ??
												entry.provider ??
												'—'}
									...
									<div className="flex items-center gap-1 ml-2 shrink-0">
										<Button
											...
											title={
												entry.id === defaultConfigId
													? t('settings:ai.removeDefault', '取消默认')
													: t('settings:ai.setDefault', '设为默认')
											}
											onClick={() =>
												setDefaultMutation.mutate(entry.id === defaultConfigId ? null : entry.id)
											}
										>
											<Star
												className={`h-3.5 w-3.5 ${entry.id === defaultConfigId ? 'fill-current' : ''}`}
											/>
										</Button>
										<Button
											variant="ghost"
				...
				<DialogContent className="sm:max-w-xl">
					...
					<form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
						...
						<div className="space-y-1.5">
							...
							<Select value={watch('locale') || 'zh'} onValueChange={(v) => setValue('locale', v)}>
								<SelectTrigger className="h-8 text-xs cursor-pointer">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
