...
test('theme customizer card 将自定义主色操作行统一为 sm 高度', () => {
	...
	assert.equal(file.includes('className="h-8 w-12 shrink-0 cursor-pointer rounded-xl border-border/70 bg-background p-1"'), true);
	assert.equal(file.includes('className="h-8 w-[8.5rem] shrink-0 rounded-xl border-border/70 bg-background uppercase tracking-[0.16em]"'), true);
