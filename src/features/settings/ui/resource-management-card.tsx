			...
			<div className="space-y-2">
				...
				{geoItems.map((item) => (
					<div key={item.id} className="rounded-xl border border-border/70 bg-background/70 p-3">
						<div className="flex items-center justify-between gap-2">
							<div className="min-w-0">
								<p className="truncate text-sm font-medium">GEO 数据库 {item.fileName}</p>
								<p className="truncate text-xs text-muted-foreground">{item.localPath || '未下载到本地'}</p>
							</div>
							<div className="flex items-center gap-1">
								{item.installed ? <Badge variant="outline">已下载</Badge> : <Badge variant="secondary">未下载</Badge>}
							</div>
						</div>
						...
						{resourceProgress?.resourceId === item.id ? (
							<div className="mt-2 space-y-1">
								<div className="flex items-center justify-between text-[11px] text-muted-foreground">
									<span>{resourceProgress.stage === 'error' ? '失败' : resourceProgress.stage === 'done' ? '已完成' : '下载中'}</span>
									<span>{resourceProgress.percent === null ? '--' : `${Math.floor(resourceProgress.percent)}%`}</span>
								</div>
								<div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
									<div
