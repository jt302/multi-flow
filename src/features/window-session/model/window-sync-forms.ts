...
export function createArrangeWindowsFormSchema() {
	return z.object({
		...
		if (data.mode === 'cascade') {
			if (!data.width) {
				ctx.addIssue({ code: z.ZodIssueCode.custom, message: i18next.t('window:validation.widthPositive'), path: ['width'] });
			}
			if (!data.height) {
				ctx.addIssue({ code: z.ZodIssueCode.custom, message: i18next.t('window:validation.heightPositive'), path: ['height'] });
			}
		}
	});
}
