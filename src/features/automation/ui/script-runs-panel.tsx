...
import { clearAutomationRuns, deleteAutomationRun } from '@/entities/automation/api/automation-api';
...
export function ScriptRunsPanel({ runs, scriptId, scriptName, onRunsChange }: Props) {
				...
				<ScrollArea className="h-full">
					{runs.length === 0 ? (
						<div className="text-center py-8">
							...
							<p className="text-xs text-muted-foreground mt-1">{t('common:runScriptHint')}</p>
						</div>
					) : (
					...
					<SheetHeader className="px-4 py-3 border-b shrink-0">
						<SheetTitle className="flex items-center gap-2 text-sm font-medium">
							...
							<span className="text-muted-foreground font-normal">
								{sheetRun ? new Date(sheetRun.startedAt * 1000).toLocaleString() : ''}
							</span>
							{sheetRun?.finishedAt && (
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>{t('common:confirmDeleteRecord')}</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel asChild>
							<Button type="button" variant="ghost" className="cursor-pointer">
