...
test('fs preferences drawer keeps dialog content scrollable when height is constrained', () => {
	assert.equal(
		source.includes('grid-rows-[auto_minmax(0,1fr)]'),
		true,
	);
	assert.equal(
...
test('fs preferences drawer uses a consistent whitelist form grid and aligns action buttons', () => {
	assert.equal(
		source.includes('sm:grid-cols-[168px_minmax(0,1fr)]'),
		true,
	);
	assert.equal(
		source.includes("name=\"path\""),
		true,
	);
	assert.equal(
		source.includes("placeholder={t('fileSystem.pathPlaceholder')}"),
		true,
	);
	assert.equal(
		source.includes("className=\"shrink-0\""),
		true,
	);
});
