...
test('table dialog is not locked to max-w-2xl', () => {
	const tableBlock = source.split("if (request.dialogType === 'table') {")[1]?.split('// ─── image 类型')[0] ?? '';

	assert.equal(tableBlock.includes('className="max-w-2xl"'), false);
	assert.equal(
		tableBlock.includes('className="grid max-h-[85vh] w-[min(96vw,1200px)] max-w-none grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden p-0"'),
		true,
	);
});
...
test('table dialog uses inner scroll containers for wide tables', () => {
	...
	assert.equal(tableBlock.includes('className="max-w-[240px] break-words whitespace-normal align-top"'), true);
});
