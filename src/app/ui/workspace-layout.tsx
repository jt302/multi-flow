...
import type { NavId, WorkspaceOutletContext } from '@/app/model/workspace-types';
...
export function WorkspaceLayout() {
	...
	const profileNavigationIntent = useWorkspaceNavigationStore((state) => state.profileNavigationIntent);
	const setProfileNavigationIntent = useWorkspaceNavigationStore((state) => state.setProfileNavigationIntent);
	const clearProfileNavigationIntent = useWorkspaceNavigationStore((state) => state.clearProfileNavigationIntent);
	...
	const outletContext: WorkspaceOutletContext = buildWorkspaceLayoutOutletContext({
		activeNav,
		theme: {
				...
				<section className="flex h-full min-h-0 flex-col gap-3">
					...
					<Card className="min-h-0 flex-1 overflow-hidden border-border/60 bg-card/84 p-4 backdrop-blur-2xl md:p-5">
						<div className="h-full min-h-0 overflow-y-auto pr-1">
							<Suspense
								fallback={<Card className="p-6 text-sm text-muted-foreground">页面加载中...</Card>}
							>
