...
test('workspace topbar keeps sidebar trigger and shows current page title/description on the left', () => {
	...
	assert.equal(
		source.includes(
			'flex w-full min-w-0 items-center gap-2 sm:ml-auto sm:w-auto sm:max-w-full sm:shrink-0',
		),
		true,
	);
	assert.equal(
		source.includes('min-w-0 flex-1 justify-start border-border/40 bg-background/50'),
		true,
	);
	assert.equal(source.includes('min-w-0 flex-1 justify-start bg-background/50'), true);
	assert.equal(
		source.includes('shrink-0 rounded-xl border border-border/40 bg-background/50'),
		true,
	);
