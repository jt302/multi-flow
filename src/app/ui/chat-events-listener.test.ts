...
test('chat events listener does not invalidate chat messages for every live message event', () => {
	...
	const messageListenerBlock =
		source
			.split("listen<ChatMessageEvent>('ai_chat://message', (event) => {")[1]
			?.split('}).then((unlisten) => {')[0] ?? '';

	assert.equal(source.includes("listen<ChatMessageEvent>('ai_chat://message'"), true);
	assert.equal(
		messageListenerBlock.includes('queryKeys.chatMessages(event.payload.sessionId)'),
		false,
	);
