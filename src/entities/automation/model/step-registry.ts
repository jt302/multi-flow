...
export const STEP_TOOL_INFO: Record<string, StepToolInfo> = {
	...
	magic_get_browser: {
		...
		inputs: [
			{ name: 'browser_id', desc: '目标浏览器窗口 ID', required: true },
		],
	...
	magic_click_at: {
		...
		example:
			'grid: "1000,1000", position: "500,57", button: "left", browser_id: 123',
	},
	magic_click_element: {
	...
	magic_get_ui_elements: {
		description: '读取浏览器 UI 当前状态，返回工具栏按钮、标签页列表和菜单信息，供脚本先判断再决策。',
		inputs: [
			{ name: 'browser_id', desc: '目标浏览器窗口 ID', required: false },
		],
	...
	magic_query_dom: {
		...
		inputs: [
			{ name: 'by', desc: '查询方式：css/xpath/text/aria/role/placeholder/name/search/idx', required: true },
	...
	magic_click_dom: {
		description: '按选择器或 idx 点击页面 DOM 元素，底层会注入真实鼠标事件而不是直接调用 el.click()。',
		inputs: [
		...
		outputs: [],
		whenToUse: '需要真正模拟用户点击网页元素，尽量绕过前端框架或反自动化对 el.click() 的限制时使用。',
		example: 'by: "idx", selector: "5"',
	},
	magic_fill_dom: {
		description: '定位并填写表单元素，支持先清空再输入，适用于 input、textarea 等表单控件。',
		inputs: [
			{ name: 'by', desc: '定位方式，支持 css/text/placeholder/name/search/idx 等', required: true },
			{ name: 'selector', desc: '选择器表达式或 idx 值', required: true },
		...
		outputs: [],
		whenToUse: '自动填写登录框、搜索框或表单字段时使用；配合 query_dom / get_page_content 能更稳地选中目标。',
		example: 'by: "css", selector: "#username", value: "myuser", clear: true',
	},
	magic_send_keys: {
		description: '发送键盘序列，支持浏览器快捷键、通用修饰键组合、特殊键以及普通文本输入。',
		inputs: [
			{ name: 'keys', desc: '按键数组，如 ["meta+l", "https://example.com", "Return"]', required: true },
			{ name: 'tab_id', desc: '目标标签页 ID，省略时使用活动标签页', required: false },
		],
		outputs: [],
	...
	magic_get_page_info: {
		...
		outputs: [{ name: 'output_key', desc: '页面状态对象，含 url/title/loading/ready_state/scroll 等字段' }],
		whenToUse: '执行页面动作前后做状态确认，判断页面是否加载完成、是否出现对话框，或记录当前滚动位置时使用。',
		example: 'tab_id: 456',
	},
	magic_scroll: {
	...
	magic_get_page_content: {
		description: '获取页面语义快照，可返回交互元素、文本内容、无障碍树或完整页面结构，并自动生成可复用的 idx 引用。',
		inputs: [
		...
		outputs: [{ name: 'output_key', desc: '页面语义快照；interactive/full 模式下会返回 idx 引用信息' }],
		whenToUse: '给 AI 或脚本先“读懂页面”再决定下一步操作，特别适合复杂表单、长页面或需要节省 token 的场景。',
