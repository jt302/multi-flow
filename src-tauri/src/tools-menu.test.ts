...
test('tools menu exposes devtools and reload entries for the main window', () => {
	...
	assert.equal(source.includes('MenuItemBuilder::with_id(MENU_ID_OPEN_DEVTOOLS, "打开开发者调试工具")'), true);
	assert.equal(source.includes('MenuItemBuilder::with_id(MENU_ID_RELOAD_MAIN_WINDOW, "刷新")'), true);
});
