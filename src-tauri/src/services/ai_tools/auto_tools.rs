...
pub async fn execute(name: &str, args: Value, ctx: &mut ToolContext<'_>) -> Result<ToolResult, String> {
    ...
    match name {
        ...
        "auto_list_scripts" => {
            let scripts = state.lock_automation_service()
                .list_scripts()
                .map_err(|e| e.to_string())?;
            // 返回摘要（不含完整步骤内容，减少 token）
            let summaries: Vec<Value> = scripts.iter().map(|s| json!({
                "id": s.id,
                "name": s.name,
            ...
            Ok(ToolResult::text(serde_json::to_string(&summaries).unwrap_or_default()))
        }

        "auto_get_script" => {
            let script_id = require_str(&args, "script_id")?;
            let script = state.lock_automation_service()
                .get_script(&script_id)
                .map_err(|e| e.to_string())?;
            Ok(ToolResult::text(serde_json::to_string(&script).unwrap_or_default()))
        }

        "auto_create_script" => {
            let name_val = require_str(&args, "name")?;
            let description = opt_str(&args, "description");
            let associated_profile_ids = args.get("associated_profile_ids")
        ...
        "auto_update_script" => {
            let script_id = require_str(&args, "script_id")?;
            // 先获取现有脚本，叠加更新字段
            let existing = state.lock_automation_service()
                .get_script(&script_id)
                .map_err(|e| e.to_string())?;
            ...
            let updated = state.lock_automation_service()
        ...
        "auto_run_script" => {
            ...
            let initial_vars: Option<std::collections::HashMap<String, String>> =
                args.get("initial_vars")
                    .and_then(|v| serde_json::from_value(v.clone()).ok());
        ...
        "auto_list_runs" => {
            let script_id = require_str(&args, "script_id")?;
            let runs = state.lock_automation_service()
                .list_runs(&script_id)
                .map_err(|e| e.to_string())?;
            Ok(ToolResult::text(serde_json::to_string(&runs).unwrap_or_default()))
        }
        ...
        "auto_list_active_runs" => {
            ...
            Ok(ToolResult::text(serde_json::to_string(&run_ids).unwrap_or_default()))
        }

        "auto_get_run" => {
            let run_id = require_str(&args, "run_id")?;
            let run = state.lock_automation_service()
                .get_run(&run_id)
                .map_err(|e| e.to_string())?;
            Ok(ToolResult::text(serde_json::to_string(&run).unwrap_or_default()))
        }
        ...
        "auto_cancel_run" => {
            ...
            Ok(ToolResult::text(format!("Run '{}' cancel requested", run_id)))
        }

        // ═══════════════ AI 配置管理 ═══════════════

        "auto_list_ai_configs" => {
            let configs = state.app_preference_service
                .lock()
                .unwrap_or_else(|e| e.into_inner())
                .list_ai_configs()
                .map_err(|e| e.to_string())?;
            // 脱敏 API Key：仅显示末 4 位
            let masked: Vec<Value> = configs.iter().map(|c| json!({
                "id": c.id,
                "name": c.name,
                "provider": c.provider,
                "base_url": c.base_url,
            ...
            Ok(ToolResult::text(serde_json::to_string(&masked).unwrap_or_default()))
        }
        ...
        "auto_create_ai_config" => {
            ...
            let created = state.app_preference_service
                .lock()
                .unwrap_or_else(|e| e.into_inner())
                .create_ai_config(entry)
                .map_err(|e| e.to_string())?;
            Ok(ToolResult::text(serde_json::to_string(&created).unwrap_or_default()))
        }
        ...
        "auto_create_captcha_config" => {
            ...
            let entry = CaptchaSolverConfig {
                ...
                api_key,
                base_url: opt_str(&args, "base_url"),
                is_default: args.get("is_default").and_then(|v| v.as_bool()).unwrap_or(false),
            };
            let created = state.app_preference_service
                .lock()
                .unwrap_or_else(|e| e.into_inner())
