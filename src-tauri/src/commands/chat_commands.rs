...
pub async fn send_chat_message(
    ...
    // 获取会话配置
    let (profile_id, ai_config_id, system_prompt, tool_categories, profile_ids, active_profile_id) = {
        let chat_svc = state.chat_service.lock().unwrap_or_else(|p| p.into_inner()).clone();
        let session = chat_svc.get_session(&session_id).await.map_err(|e| e.to_string())?;
        (
            session.profile_id,
    ...
    // 加载 AI 配置
    let ai_config = {
        let pref_svc = state.app_preference_service.lock().unwrap_or_else(|p| p.into_inner());
        if let Some(ref cid) = ai_config_id {
            pref_svc
                .find_ai_config_by_id(cid)
                .map_err(|e| e.to_string())?
                .map(|e| crate::services::chat_execution_service::ai_config_entry_to_provider_config(&e))
                .unwrap_or_default()
        } else {
            // 使用第一个可用配置
            pref_svc
    ...
    let has_profile = profile_id.is_some()
        || profile_ids.as_ref().map_or(false, |ids| !ids.is_empty());
...
pub fn stop_chat_generation(state: State<'_, AppState>, session_id: String) -> Result<(), String> {
    let mut tokens = state.chat_cancel_tokens.lock().unwrap_or_else(|p| p.into_inner());
...
pub async fn regenerate_chat_message(
    ...
    let (last_user_text, sort_order) = {
        let chat_svc = state.chat_service.lock().unwrap_or_else(|p| p.into_inner()).clone();
        let messages = chat_svc.list_messages(&session_id).await.map_err(|e| e.to_string())?;
        let last_user = messages
            ...
            .clone();
        (last_user.content_text.unwrap_or_default(), last_user.sort_order)
    };

    // 删除最后一条用户消息之后（含）的所有消息
    {
        let chat_svc = state.chat_service.lock().unwrap_or_else(|p| p.into_inner()).clone();
        chat_svc.delete_messages_from(&session_id, sort_order).await.map_err(|e| e.to_string())?;
    }

    // 重新发送
    Box::pin(send_chat_message(app, state, session_id, last_user_text)).await
}
...
pub async fn test_ai_connection(
    ...
    let config_entry = {
        let pref = state.app_preference_service.lock().unwrap_or_else(|p| p.into_inner());
        let all_configs = pref.list_ai_configs().map_err(|e| e.to_string())?;
        all_configs.into_iter().find(|c| c.id == config_id)
            .ok_or_else(|| "AI config not found".to_string())?
    };
    let provider_config = crate::services::chat_execution_service::ai_config_entry_to_provider_config(&config_entry);
...
fn get_tool_metadata(name: &str) -> (String, String) {
    match name {
        // 应用级别操作
        "app_delete_profile" => ("删除环境".to_string(), "永久删除指定的浏览器环境配置".to_string()),
        "app_delete_proxy" => ("删除代理".to_string(), "永久删除指定的代理配置".to_string()),
        "app_delete_group" => ("删除分组".to_string(), "永久删除指定的环境分组".to_string()),
        "app_stop_profile" => ("停止环境".to_string(), "强制停止正在运行的浏览器环境".to_string()),
        "app_stop_all_profiles" => ("停止全部环境".to_string(), "强制停止所有正在运行的浏览器环境".to_string()),
        "app_purge_profile" => ("清空环境回收站".to_string(), "彻底删除回收站中的环境，无法恢复".to_string()),
        "app_purge_proxy" => ("清空代理回收站".to_string(), "彻底删除回收站中的代理配置".to_string()),
        "app_purge_group" => ("清空分组回收站".to_string(), "彻底删除回收站中的分组配置".to_string()),

        // Magic 控制操作
        "magic_set_closed" => ("关闭 Magic".to_string(), "关闭 Magic 控制通道，停止浏览器扩展通信".to_string()),
        "magic_safe_quit" => ("安全退出 Magic".to_string(), "优雅关闭 Magic 控制器，保存当前状态".to_string()),
        ...
        // 浏览器数据清除
        "cdp_clear_cookies" => ("清除 Cookies".to_string(), "清除浏览器的所有 Cookie 数据".to_string()),
        "cdp_clear_local_storage" => ("清除 Local Storage".to_string(), "清除浏览器的本地存储数据".to_string()),
        "cdp_clear_session_storage" => ("清除 Session Storage".to_string(), "清除浏览器的会话存储数据".to_string()),

        // 自动化脚本操作
        "auto_delete_script" => ("删除脚本".to_string(), "永久删除指定的自动化脚本".to_string()),
...
/// 获取所有危险工具的权限设置
#[tauri::command]
pub fn get_tool_permissions(state: State<'_, AppState>) -> Result<Vec<ToolPermissionEntry>, String> {
    let pref_svc = state.app_preference_service.lock().unwrap_or_else(|p| p.into_inner());
...
pub fn set_tool_permission(
    ...
    let pref_svc = state.app_preference_service.lock().unwrap_or_else(|p| p.into_inner());
    pref_svc.set_tool_confirmation_override(&tool_name, require_confirmation)
        .map_err(|e| e.to_string())
}
...
pub fn set_all_tool_permissions(
    ...
    let pref_svc = state.app_preference_service.lock().unwrap_or_else(|p| p.into_inner());
    pref_svc.set_all_tool_confirmation_overrides(require_confirmation)
        .map_err(|e| e.to_string())
}
